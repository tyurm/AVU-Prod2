page 82092 "AVU Table Records"
{
        ...
        area(processing)
        {
            group(Process)
            {
                action("Delete Records")
                {
                    ...
                    var
                        Int: Record Integer;
                        ChangeLogEntryTemp: Record "Change Log Entry" temporary;
                        RecRef: RecordRef;
                        EntryNo: Integer;
                    begin
                        CurrPage.SetSelectionFilter(Int);
                        Message(Format(Int.Count()));
                        RecRef.Open(TableNo);
                        if Int.FindSet() then
                            repeat
                                RecRef.FindFirst();
                                RecRef.Next(Int.Number - 1);
                                ChangeLogEntryTemp.Init();
                                EntryNo += 1;
                                ChangeLogEntryTemp."Entry No." := EntryNo;
                                ChangeLogEntryTemp."Record ID" := RecRef.RecordId();
                                ChangeLogEntryTemp.Insert();
                            until Int.Next() = 0;
                        if ChangeLogEntryTemp.FindSet() then
                            repeat
                                RecRef.Get(ChangeLogEntryTemp."Record ID");
                                RecRef.Delete();
                                ChangeLogEntryTemp.Delete();
                            until ChangeLogEntryTemp.Next() = 0;
                        NoRecords := RecRef.Count();
                        SetRange(Number, 1, NoRecords);
                        RecRef.Close();
                    end;
                }
                /*action(ProcessData)
    ...
    begin
        RecRef.FindFirst();
        RecRef.Next(Number - 1);
        for i := 1 to NoFields do begin
            FieldRef := RecRef.Field(MatrixColumnNo[i]);
            MatrixCellData[i] := Format(FieldRef.Value());
        end;
    end;

    trigger OnOpenPage()
    begin
        SetFieldsVisibility(NoFields);
        RecRef.Open(TableNo);
        NoRecords := RecRef.Count();
        SetRange(Number, 1, NoRecords);
    end;
    ...
    var
        ...
        MatrixColumnNo: array[1000] of Integer;
    ...
    procedure Load(TableNo_: Integer)
    begin
        TableNo := TableNo_;
        GetFields(TableNo);
    end;

    local procedure GetFields(TableNo: Integer): Boolean
    var
        Fields: Record Field;
        i: Integer;
    begin
        ...
        if Fields.FindSet() then
            repeat
                i := i + 1;
                MatrixColumnNo[i] := Fields."No.";
                MatrixColumnCaptions[i] := Fields.FieldName;
            until Fields.Next() = 0;
        NoFields := i;
    end;
