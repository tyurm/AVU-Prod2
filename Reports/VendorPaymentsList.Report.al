report 82003 "AVU Vendor Payments List"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/Layouts/AVUVendorPaymentsList.rdlc';
    ApplicationArea = Basic, Suite;
    Caption = 'AVU Vendor Payments List';
    ...
    dataset
    {
        dataitem("Vendor Ledger Entry"; "Vendor Ledger Entry")
        {
            ...
            column(Text003___SELECTSTR_Layout___1_Text006_; Text003Lbl + SelectStr(LayoutOption + 1, Text006Lbl))
            {
            }
            column(Text004___GETFILTERS; Text004Lbl + GetFilters())
            {
            }
            ...
            column(Text005_________Vendor__No____________Vendor_Name; Text005Lbl + ' ' + Vendor."No." + ', ' + Vendor.Name)
            {
            }
            ...
            begin
                ...
                if LayoutOption = LayoutOption::"FCY Amounts" then begin
                    ...
                    Status := CopyStr(Text000Lbl, 1, MaxStrLen(Status));
                    if Open then
                        if "Remaining Amt. (LCY)" = "Amount (LCY)" then
                            Status := CopyStr(Text001Lbl, 1, MaxStrLen(Status))
                        else
                            Status := CopyStr(Text002Lbl, 1, MaxStrLen(Status));
                end;

                if LayoutOption = LayoutOption::Standard then begin
                ...
                if SortingOption = SortingOption::Vendor then begin
            ...
            trigger OnPreDataItem()
            begin
                LayoutInt := LayoutOption;
                SortInt := SortingOption;

                if SortingOption = SortingOption::Chronological then
    ...
    var
        ...
        SortingOption: Option Vendor,Chronological;
        LayoutOption: Option Standard,"FCY Amounts","Posting Info";
        LinesPerGrp: Integer;
        NoOfPmts: Integer;
        AccNo: Text[20];
        AccName: Text[100];
        Status: Text[8];
        Exrate: Decimal;
        Text000Lbl: Label 'Closed';
        Text001Lbl: Label 'Open';
        Text002Lbl: Label 'Partial';
        Text003Lbl: Label 'Layout ';
        Text004Lbl: Label 'Filter: ';
        Text005Lbl: Label 'Total Vendor';
        Text006Lbl: Label 'Standard,FCY Amounts,Posting Info';
        PmtDiscLCY: Decimal;
        PaymentLCY: Decimal;
        LayoutInt: Integer;
        SortInt: Integer;
